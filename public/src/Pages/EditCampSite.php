...
<body>
  ...
  if (isset($_POST['SubmitForm'])) {
    ...
    $FormData['directions'] = htmlspecialchars($cGOAT->GetFormData('Notes'), ENT_QUOTES, 'UTF-8');

    if ($cGOAT->UpdateSite($FormData)) {
      // Record has been updated in database, now create an audit trail
      $cGOAT->CreateAudit($SiteOldData, $FormData);
       header("Location: index.php?page=displaycampsite&Siteid=" . $FormData['IDX']);
    }
  }
  ...
  ?>
    <div class="row flex-wrap">
      ...
      ?>
        <center>
          <p>You must have an account with the GOAT website and be logged on to edit a campsite.</p>
          <p>Or use the contact link and provide us the information and we will update the campsite.</p>
          <a class="btn btn-primary btn-sm" href="index.php?page=login">Log On</a>
        </center>
      <?php
      } else {
        // Display a form to edit the camp site.
      ?>
        <div class="form-coach px-5" style="background-color: var(--scouting-lighttan);">
          ...
          <form action="index.php?page=editcampsites" id="campsite-form" method="post">
            <div class="form-row">
              <div class="col-3">
            ...
            <div class="form-row">
              <div class="col-12">
                ...
                <textarea class="form-control" name="Notes" id="Notes" rows="10" style="height:100%;"><?php if (strlen($Site['directions']) > 0) echo $Site['directions']; ?></textarea>
              </div>
            </div>
            <div class="form-row">
