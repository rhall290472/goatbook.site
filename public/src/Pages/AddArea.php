...
<body>
  ...
  <div class="row flex-wrap">
    <div class="form-coach px-5" style="background-color: var(--scouting-lighttan);">
      <p style="text-align:Left"><b>Add New Area</b></p>
      <form action="index.php?page=addarea" id="coach-form" method="post">
        <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token'] ?? bin2hex(random_bytes(32))); ?>">
        <div class="form-row">
          <div class="col-3">
            <label for=element_1_1>New Area</label>
            <input type="text" name="element_1_1" class="form-control" required />
          </div>
          <div class="col-3">
            <label for=element_1_1>Curent Areas</label>
            <?php $cGOAT->DisplayArea(0, "element_1_2"); ?>
          </div>
        </div>
        <div class="form-row">
